...
import { AddshippingTramaolcodComponent } from './add-shiping-agency/addshipping.component';
import { AddtrakingTramaolcodComponent } from './add-traking-modals/Addtraking.component';
import { ComposeMailBodyComponent } from '../allorder/compose-email-modal/composebody.component';

@Component({
  selector: 'app-buytraolcod',
  templateUrl: './buytraolcod.component.html',
  styleUrls: ['./buytraolcod.component.css']
})
export class BuytraolcodComponent implements OnInit {
  searchKey: any;
  resposeData: any;
  selectrow: any;
  userData = {};
  ...
  ngOnInit() {
    this.painserv.postData(this.userData, "getrecordstronlinecod").then((result) => {
  ...
  ExportTOExcel() {
    this.excelService.exportAsExcelFile(this.resposeData.userData, 'Traonlinecod');
  }
  ...
  deleteOrder(inx) {
    if (confirm("Are you sure to delete this Order?")) {
      ...
      this.painserv.postData({ 'id': inx }, "dlttramaolcod").then((result) => {
        this.resposeData = result;
  ...
  openDialog(inx): void {
    ...
    const dialogRef = this.dialog.open(DetailsDialogTramaolcod, {
      width: '900px',
      data: { array: array }
    });
  ...
  updateStatus(oid: any, pstsval: any) {
    let stsVal: any;
    const bottmSheet = this.dialog.open(OcStatusUpdateBottomSheetTramaolcod, {
      width: '300px',
    ...
    bottmSheet.afterClosed().subscribe(result => {
      if (result) {
        this.loadSts = true;
        this.painserv.postData({ 'oid': oid, "statusval": result }, "updatestatustraonlinecod").then((result) => {
          this.resposeData = result;
  ...
  addshipping(oid: any): void {
    var shipp: any;
    const addtrkmodal = this.dialog.open(AddshippingTramaolcodComponent, {
      width: '300px',
      data: { shipp: shipp }
    });
    ...
    addtrkmodal.afterClosed().subscribe(result => {
      if (result) {
        this.loadSts = true;
        this.painserv.postData({ 'oid': oid, "shipp": result }, "addshippingtraolcod").then((result) => {
          this.resposeData = result;
  ...
  addTrakingid(oid: any): void {
    var tids = [{ "trakingid": "" }];
    const addtrkmodal = this.dialog.open(AddtrakingTramaolcodComponent, {
      width: '400px',
      data: { tids: tids }
    });
    ...
    addtrkmodal.afterClosed().subscribe(result => {
      if (result) {
        this.loadSts = true;
        this.painserv.postData({ 'oid': oid, "trakingids": result }, "addtrakingidtraolcod").then((result) => {
          this.resposeData = result;
...
export class DetailsDialogTramaolcod {
  total: any;
  stslod = false;
  discount : any;
  subtot: any;
  constructor(
    public dialogRef: MatDialogRef<DetailsDialogTramaolcod>,
  ...
  addTraking(oid: any): void {
    ...
    addtrkmodal.afterClosed().subscribe(result => {
      if(result) {
        this.painserv.postData({ 'oid': oid, "trakingids": result }, "addtrakingidtramasale").then((result) => {
...
export class OcStatusUpdateBottomSheetTramaolcod {
  ststusList = [];
  statusVal: any;
  constructor(
    public dialogRef: MatDialogRef<OcStatusUpdateBottomSheetTramaolcod>,
