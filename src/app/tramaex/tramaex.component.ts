...
import { AddshippingTramaExComponent } from './add-shiping-agency/addshipping.component';
import { AddtrakingTramaExComponent } from './add-traking-modals/Addtraking.component';
import { ComposeMailBodyComponent } from '../allorder/compose-email-modal/composebody.component';

@Component({
  selector: 'app-tramaex',
  templateUrl: './tramaex.component.html',
  styleUrls: ['./tramaex.component.css']
})
export class TramaexComponent implements OnInit {
  ...
  ngOnInit() {
    this.painserv.postData(this.userData, "getrecordstramaex").then((result) => {
  ...
  ExportTOExcel() {
    this.excelService.exportAsExcelFile(this.resposeData.userData, 'TramadolExport');
  }
  ...
  deleteOrder(inx) {
    if (confirm("Are you sure to delete this Order?")) {
      ...
      this.painserv.postData({ 'id': inx }, "dltTramaEx").then((result) => {
        this.resposeData = result;
  ...
  openDialog(inx): void {
    ...
    const dialogRef = this.dialog.open(DetailsDialogTramaEx, {
      width: '900px',
      data: { array: array }
    });
  ...
  updateStatus(oid: any, pstsval: any) {
    let stsVal: any;
    const bottmSheet = this.dialog.open(OcStatusUpdateBottomSheetTramaEx, {
      width: '300px',
    ...
    bottmSheet.afterClosed().subscribe(result => {
      if (result) {
        this.loadSts = true;
        this.painserv.postData({ 'oid': oid, "statusval": result }, "updatestatusortramaex").then((result) => {
          this.resposeData = result;
  ...
  addshipping(oid: any): void {
    var shipp: any;
    const addtrkmodal = this.dialog.open(AddshippingTramaExComponent, {
      width: '300px',
      data: { shipp: shipp }
    });
    ...
    addtrkmodal.afterClosed().subscribe(result => {
      if (result) {
        this.loadSts = true;
        this.painserv.postData({ 'oid': oid, "shipp": result }, "addshippingtramaex").then((result) => {
          this.resposeData = result;
  ...
  addTrakingid(oid: any): void {
    var tids = [{ "trakingid": "" }];
    const addtrkmodal = this.dialog.open(AddtrakingTramaExComponent, {
      width: '400px',
      data: { tids: tids }
    });
    ...
    addtrkmodal.afterClosed().subscribe(result => {
      if (result) {
        this.loadSts = true;
        this.painserv.postData({ 'oid': oid, "trakingids": result }, "addtrakingidtramaex").then((result) => {
          this.resposeData = result;
...
export class DetailsDialogTramaEx {
  total: any;
  stslod = false;
  discount : any;
  subtot: any;
  constructor(
    public dialogRef: MatDialogRef<DetailsDialogTramaEx>,
...
export class OcStatusUpdateBottomSheetTramaEx {
  ststusList = [];
  statusVal: any;
  constructor(
    public dialogRef: MatDialogRef<OcStatusUpdateBottomSheetTramaEx>,
