...
import { AddtrakingpainComponent } from './add-traking-modals/Addtraking.component';
import { AddshippingComponent } from './add-shiping-agency/addshipping.component';
...
@Component({
  ...
  styleUrls: ['./painkiller.component.css']
})
export class PainkillerComponent implements OnInit {
  searchKey: any;
  resposeData: any;
  selectrow: any;
  userData = {};
  ...
  ngOnInit() {
    this.painserv.postData(this.userData, "getrecordspain").then((result) => {
  ...
  /*openDialog(inx) {
    console.log(this.resposeData.userData[inx]);
    alert(inx);
  }*/
  ...
  ExportTOExcel() {
    ...
    this.excelService.exportAsExcelFile(this.resposeData.userData, 'PainKiller');
  ...
  deleteOrder(inx) {
    if (confirm("Are you sure to delete this Order?")) {
      ...
      this.painserv.postData({ 'id': inx }, "dltPainkillOrder").then((result) => {
        this.resposeData = result;
  ...
  updateStatus(oid: any, pstsval: any) {
    let stsVal: any;
    const bottmSheet = this.dialog.open(OcStatusUpdateBottomSheetPain, {
      width: '300px',
    ...
    bottmSheet.afterClosed().subscribe(result => {
      if (result) {
        this.loadSts = true;
        this.painserv.postData({ 'oid': oid, "statusval": result }, "updatestatusorpain").then((result) => {
          this.resposeData = result;
  ...
  addshipping(oid: any): void {
    var shipp: any;
    const addtrkmodal = this.dialog.open(AddshippingComponent, {
      width: '300px',
      data: { shipp: shipp }
    });
    ...
    addtrkmodal.afterClosed().subscribe(result => {
      if (result) {
        this.loadSts = true;
        this.painserv.postData({ 'oid': oid, "shipp": result }, "addshippingpain").then((result) => {
          this.resposeData = result;
  ...
  addTrakingid(oid: any): void {
    var tids = [{ "trakingid": "" }];
    const addtrkmodal = this.dialog.open(AddtrakingpainComponent, {
      width: '400px',
      data: { tids: tids }
    });
...
export class OcStatusUpdateBottomSheetPain {
  ststusList = [];
  statusVal: any;
  constructor(
    public dialogRef: MatDialogRef<OcStatusUpdateBottomSheetPain>,
