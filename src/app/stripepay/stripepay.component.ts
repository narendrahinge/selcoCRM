...
import { AddshippingStripepayComponent } from './add-shiping-agency/addshipping.component';
import { AddtrakingStripepayComponent } from './add-traking-modals/Addtraking.component';
import { ComposeMailBodyComponent } from '../allorder/compose-email-modal/composebody.component';

@Component({
  selector: 'app-stripepay',
  templateUrl: './stripepay.component.html',
  styleUrls: ['./stripepay.component.css']
})
export class StripepayComponent implements OnInit {
  ...
  ngOnInit() {
    this.painserv.postData(this.userData, "getrecordstripepay").then((result) => {
  ...
  ExportTOExcel() {
    this.excelService.exportAsExcelFile(this.resposeData.userData, 'StripeMarchanSheet');
  }
  ...
  deleteOrder(inx) {
    if (confirm("Are you sure to delete this Order?")) {
      ...
      this.painserv.postData({ 'id': inx }, "dltStripepay").then((result) => {
        this.resposeData = result;
    ...
    openDialog(inx): void {
      ...
      const dialogRef = this.dialog.open(DetailsDialogStripePay, {
        width: '900px',
        data: { array: array }
      });
    ...
    updateStatus(oid: any, pstsval: any) {
      let stsVal: any;
      const bottmSheet = this.dialog.open(OcStatusUpdateBottomSheetStripepay, {
        width: '300px',
      ...
      bottmSheet.afterClosed().subscribe(result => {
        if (result) {
          this.loadSts = true;
          this.painserv.postData({ 'oid': oid, "statusval": result }, "updatestatusstripepay").then((result) => {
            this.resposeData = result;
    ...
    addshipping(oid: any): void {
      var shipp: any;
      const addtrkmodal = this.dialog.open(AddshippingStripepayComponent, {
        width: '300px',
        data: { shipp: shipp }
      });
      ...
      addtrkmodal.afterClosed().subscribe(result => {
        if (result) {
          this.loadSts = true;
          this.painserv.postData({ 'oid': oid, "shipp": result }, "addshippingstripepay").then((result) => {
            this.resposeData = result;
    ...
    addTrakingid(oid: any): void {
      var tids = [{ "trakingid": "" }];
      const addtrkmodal = this.dialog.open(AddtrakingStripepayComponent, {
        width: '400px',
        data: { tids: tids }
      });
      ...
      addtrkmodal.afterClosed().subscribe(result => {
        if (result) {
          this.loadSts = true;
          this.painserv.postData({ 'oid': oid, "trakingids": result }, "addtrakingidstripepay").then((result) => {
            this.resposeData = result;
...
export class DetailsDialogStripePay {
  total: any;
  stslod = false;
  discount : any;
  subtot: any;
  constructor(
    public dialogRef: MatDialogRef<DetailsDialogStripePay>,
...
export class OcStatusUpdateBottomSheetStripepay {
  ststusList = [];
  statusVal: any;
  constructor(
    public dialogRef: MatDialogRef<OcStatusUpdateBottomSheetStripepay>,
