...
import { AddshippingBuyAnxiComponent } from './add-shiping-agency/addshipping.component';
import { AddtrakingBuyAnxiComponent } from './add-traking-modals/Addtraking.component';
import { ComposeMailBodyComponent } from '../allorder/compose-email-modal/composebody.component';

@Component({
  selector: 'app-buyanxity',
  templateUrl: './buyanxity.component.html',
  styleUrls: ['./buyanxity.component.css']
})
export class BuyanxityComponent implements OnInit {
  searchKey: any;
  resposeData: any;
  selectrow: any;
  userData = {};
  ...
  ngOnInit() {
    this.painserv.postData(this.userData, "getrecordsbuyanxi").then((result) => {
  ...
  ExportTOExcel() {
    this.excelService.exportAsExcelFile(this.resposeData.userData, 'BuyAnxi');
  }
  ...
  deleteOrder(inx) {
    if (confirm("Are you sure to delete this Order?")) {
      ...
      this.painserv.postData({ 'id': inx }, "dltBuyAnxity").then((result) => {
        this.resposeData = result;
    ...
    openDialog(inx): void {
      ...
      const dialogRef = this.dialog.open(DetailsDialogBuyAnxi, {
        width: '900px',
        data: { array: array }
      });
    ...
    updateStatus(oid: any, pstsval: any) {
      let stsVal: any;
      const bottmSheet = this.dialog.open(OcStatusUpdateBottomSheetBuyAnxi, {
        width: '300px',
      ...
      bottmSheet.afterClosed().subscribe(result => {
        if (result) {
          this.loadSts = true;
          this.painserv.postData({ 'oid': oid, "statusval": result }, "updatestatusbuyanxi").then((result) => {
            this.resposeData = result;
    ...
    addshipping(oid: any): void {
      var shipp: any;
      const addtrkmodal = this.dialog.open(AddshippingBuyAnxiComponent, {
        width: '300px',
        data: { shipp: shipp }
      });
      ...
      addtrkmodal.afterClosed().subscribe(result => {
        if (result) {
          this.loadSts = true;
          this.painserv.postData({ 'oid': oid, "shipp": result }, "addshippingbuyanxi").then((result) => {
            this.resposeData = result;
    ...
    addTrakingid(oid: any): void {
      var tids = [{ "trakingid": "" }];
      const addtrkmodal = this.dialog.open(AddtrakingBuyAnxiComponent, {
        width: '400px',
        data: { tids: tids }
      });
      ...
      addtrkmodal.afterClosed().subscribe(result => {
        if (result) {
          this.loadSts = true;
          this.painserv.postData({ 'oid': oid, "trakingids": result }, "addtrakingidbuyanxi").then((result) => {
            this.resposeData = result;
...
export class DetailsDialogBuyAnxi {
  total: any;
  stslod = false;
  discount : any;
  subtot: any;
  constructor(
    public dialogRef: MatDialogRef<DetailsDialogBuyAnxi>,
...
export class OcStatusUpdateBottomSheetBuyAnxi {
  ststusList = [];
  statusVal: any;
  constructor(
    public dialogRef: MatDialogRef<OcStatusUpdateBottomSheetBuyAnxi>,
