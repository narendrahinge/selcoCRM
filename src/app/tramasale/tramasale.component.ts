...
import { AddshippingTramaSaleComponent } from './add-shiping-agency/addshipping.component';
import { AddtrakingTramaSaleComponent } from './add-traking-modals/Addtraking.component';
import { ComposeMailBodyComponent } from '../allorder/compose-email-modal/composebody.component';

@Component({
  selector: 'app-tramasale',
  templateUrl: './tramasale.component.html',
  styleUrls: ['./tramasale.component.css']
})
export class TramasaleComponent implements OnInit {
  ...
  ngOnInit() {
    this.painserv.postData(this.userData, "getrecordstramasale").then((result) => {
  ...
  ExportTOExcel() {
    this.excelService.exportAsExcelFile(this.resposeData.userData, 'TramadolSale');
  }
  ...
  deleteOrder(inx) {
    if (confirm("Are you sure to delete this Order?")) {
      ...
      this.painserv.postData({ 'id': inx }, "dltTramaSale").then((result) => {
        this.resposeData = result;
    ...
    openDialog(inx): void {
      ...
      const dialogRef = this.dialog.open(DetailsDialogTramaSale, {
        width: '900px',
        data: { array: array }
      });
    ...
    updateStatus(oid: any, pstsval: any) {
      let stsVal: any;
      const bottmSheet = this.dialog.open(OcStatusUpdateBottomSheetTramaSale, {
        width: '300px',
      ...
      bottmSheet.afterClosed().subscribe(result => {
        if (result) {
          this.loadSts = true;
          this.painserv.postData({ 'oid': oid, "statusval": result }, "updatestatusortramasale").then((result) => {
            this.resposeData = result;
    ...
    addshipping(oid: any): void {
      var shipp: any;
      const addtrkmodal = this.dialog.open(AddshippingTramaSaleComponent, {
        width: '300px',
        data: { shipp: shipp }
      });
      ...
      addtrkmodal.afterClosed().subscribe(result => {
        if (result) {
          this.loadSts = true;
          this.painserv.postData({ 'oid': oid, "shipp": result }, "addshippingtramasale").then((result) => {
            this.resposeData = result;
  ...
  addTrakingid(oid: any): void {
    var tids = [{ "trakingid": "" }];
    const addtrkmodal = this.dialog.open(AddtrakingTramaSaleComponent, {
      width: '400px',
      data: { tids: tids }
    });
...
export class DetailsDialogTramaSale {
  total: any;
  stslod = false;
  discount : any;
  subtot: any;
  constructor(
    public dialogRef: MatDialogRef<DetailsDialogTramaSale>,
...
export class OcStatusUpdateBottomSheetTramaSale {
  ststusList = [];
  statusVal: any;
  constructor(
    public dialogRef: MatDialogRef<OcStatusUpdateBottomSheetTramaSale>,
