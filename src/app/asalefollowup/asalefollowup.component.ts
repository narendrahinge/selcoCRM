...
import { WatchqueueComponent } from './watch-queue-modal/addshipping.component';

@Component({
  selector: 'app-asalefollowup',
  templateUrl: './asalefollowup.component.html',
  styleUrls: ['./asalefollowup.component.css']
})
export class AsalefollowupComponent implements OnInit {
  ...
  ngOnInit() {
    this.painserv.postData(this.userData, "getaftersalesfollowuprec").then((result) => {
  ...
  openDialog(inx): void {
    var array: any = this.singleArrReturn(this.resposeData.userData, inx);
    const dialogRef = this.dialog.open(AftersalesfollowupOrderDetailsDialog, {
      width: '850px',
      data: { array: array }
    });
  ...
  watchqueue(): void {
    const dialogRef = this.dialog.open(WatchqueueComponent, {
      width: '850px',
      data: { array: this.resposeData.userData }
    });
  ...
  ExportTOExcel() {
    this.excelService.exportAsExcelFile(this.resposeData.userData, 'followuplist');
  }
  ...
  deleteOrder(inx) {
    if (confirm("Are you sure to delete this Order from followup section?")) {
      this.loadSts = true;
      this.painserv.postData({ 'id': inx }, "deletefollowupoc").then((result) => {
        this.resposeData = result;
        this.listData = new MatTableDataSource(this.resposeData.userData);
        this.listData.sort = this.sort;
        this.listData.paginator = this.paginator;
  ...
  updateStatus(oid: any, pstsval: any) {
    let stsVal: any;
    const bottmSheet = this.dialog.open(OcStatusUpdateBottomSheetfollowupoc, {
      width: '300px',
    ...
    bottmSheet.afterClosed().subscribe(result => {
      if (result) {
        if (result == "Delivered" || result == "Undelivered") {
          if (confirm("Are you sure want to make this order "+result+" beacause when you change status to "+result+" order remove from followup section?")) {
            this.loadSts = true;
            this.painserv.postData({ 'oid': oid, "statusval": result }, "changestsfollowupoc").then((result) => {
              this.resposeData = result;
              this.listData = new MatTableDataSource(this.resposeData.userData);
              this.listData.sort = this.sort;
              this.listData.paginator = this.paginator;
...
export class AftersalesfollowupOrderDetailsDialog {
  total: any;
  stslod = false;
  paylinksts = false;
  discount: any;
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<AftersalesfollowupOrderDetailsDialog>,
...
export class OcStatusUpdateBottomSheetfollowupoc {
  ststusList = [];
  statusVal: any;
  constructor(
    public dialogRef: MatDialogRef<OcStatusUpdateBottomSheetfollowupoc>,
