...
import { AddshippingTramhowto } from './add-shiping-agency/addshipping.component';
import { AddtrakingTramhowtoComponent } from './add-traking-modals/Addtraking.component';
import { ComposeMailBodyComponent } from '../allorder/compose-email-modal/composebody.component';

@Component({
  selector: 'app-tramhowto',
  templateUrl: './tramhowto.component.html',
  styleUrls: ['./tramhowto.component.css']
})
export class TramhowtoComponent implements OnInit {
  searchKey: any;
  resposeData: any;
  selectrow: any;
  userData = {};
 ...
 displayedColumns: string[] = ['inc', 'status', 'invid', 'date', 'phone', 'email', 'name', 'shsts', 'trck', 'oid'];
  ...
  ngOnInit() {
    this.painserv.postData(this.userData, "getrecordstramhowto").then((result) => {
  ...
  ExportTOExcel() {
    this.excelService.exportAsExcelFile(this.resposeData.userData, 'Tramadol-Howto');
  }
  ...
  deleteOrder(inx) {
    if (confirm("Are you sure to delete this Order?")) {
      ...
      this.painserv.postData({ 'id': inx }, "dlttramaolhowto").then((result) => {
        this.resposeData = result;
  ...
  openDialog(inx): void {
    ...
    const dialogRef = this.dialog.open(DetailsDialogTramhowto, {
      width: '900px',
      data: { array: array }
    });
  ...
  updateStatus(oid: any, pstsval: any) {
    let stsVal: any;
    const bottmSheet = this.dialog.open(OcStatusUpdateBottomSheettramhowto, {
      width: '300px',
    ...
    bottmSheet.afterClosed().subscribe(result => {
      if (result) {
        this.loadSts = true;
        this.painserv.postData({ 'oid': oid, "statusval": result }, "updatestatustramhowto").then((result) => {
          this.resposeData = result;
  ...
  addshipping(oid: any): void {
    var shipp: any;
    const addtrkmodal = this.dialog.open(AddshippingTramhowto, {
      width: '300px',
      data: { shipp: shipp }
    });
    ...
    addtrkmodal.afterClosed().subscribe(result => {
      if (result) {
        this.loadSts = true;
        this.painserv.postData({ 'oid': oid, "shipp": result }, "addshippingtramhowto").then((result) => {
          this.resposeData = result;
  ...
  addTrakingid(oid: any): void {
    var tids = [{ "trakingid": "" }];
    const addtrkmodal = this.dialog.open(AddtrakingTramhowtoComponent, {
      width: '400px',
      data: { tids: tids }
    });
    ...
    addtrkmodal.afterClosed().subscribe(result => {
      if (result) {
        this.loadSts = true;
        this.painserv.postData({ 'oid': oid, "trakingids": result }, "addtrakingidtramhowto").then((result) => {
          this.resposeData = result;
...
export class DetailsDialogTramhowto {
  total: any;
  stslod = false;
  discount : any;
  subtot: any;
  constructor(
    public dialogRef: MatDialogRef<DetailsDialogTramhowto>,
...
export class OcStatusUpdateBottomSheettramhowto {
  ststusList = [];
  statusVal: any;
  constructor(
    public dialogRef: MatDialogRef<OcStatusUpdateBottomSheettramhowto>,
