...
import { AddshippingComponentGlobal } from './add-shiping-agency/addshipping.component';
import { AddtrakinglobalpharmaComponent } from './add-traking-modals/Addtraking.component';
import { ComposeMailBodyComponent } from '../allorder/compose-email-modal/composebody.component';

@Component({
  selector: 'app-globalpharma',
  templateUrl: './globalpharma.component.html',
  styleUrls: ['./globalpharma.component.css']
})
export class GlobalpharmaComponent implements OnInit {
  ...
  ngOnInit() {
    this.painserv.postData(this.userData, "getrecordsglobalpharma").then((result) => {
  ...
  /*Click on User Details icon*/
  openDialog(inx): void {
    var array = this.resposeData.userData[inx];
    const dialogRef = this.dialog.open(DetailsdialogGlobalPharma, {
      width: '900px',
      data: { array: array }
    });
  ...
  ExportTOExcel() {
    ...
    this.excelService.exportAsExcelFile(this.resposeData.userData, 'GlobalPharma');
  ...
  deleteOrder(inx) {
    if (confirm("Are you sure to delete this Order?")) {
      ...
      this.painserv.postData({ 'id': inx }, "dltGlobalPharmaOrder").then((result) => {
        this.resposeData = result;
  ...
  updateStatus(oid: any, pstsval: any) {
    let stsVal: any;
    const bottmSheet = this.dialog.open(OcStatusUpdateBottomSheetGlobal, {
      width: '300px',
    ...
    bottmSheet.afterClosed().subscribe(result => {
      if (result) {
        this.loadSts = true;
        this.painserv.postData({ 'oid': oid, "statusval": result }, "updatestatusorglobal").then((result) => {
          this.resposeData = result;
  ...
  addshipping(oid: any): void {
    var shipp: any;
    const addtrkmodal = this.dialog.open(AddshippingComponentGlobal, {
      width: '300px',
      data: { shipp: shipp }
    });
    ...
    addtrkmodal.afterClosed().subscribe(result => {
      if (result) {
        this.loadSts = true;
        this.painserv.postData({ 'oid': oid, "shipp": result }, "addshippingglobal").then((result) => {
          this.resposeData = result;
  ...
  addTrakingid(oid: any): void {
    var tids = [{ "trakingid": "" }];
    const addtrkmodal = this.dialog.open(AddtrakinglobalpharmaComponent, {
      width: '400px',
      data: { tids: tids }
    });
    ...
    addtrkmodal.afterClosed().subscribe(result => {
      if (result) {
        this.loadSts = true;
        this.painserv.postData({ 'oid': oid, "trakingids": result }, "addtrakingidglobal").then((result) => {
          this.resposeData = result;
...
export class DetailsdialogGlobalPharma {
  total: any;
  stslod = false;
  discount: any;
  subtot: any;
  constructor(
    public dialogRef: MatDialogRef<DetailsdialogGlobalPharma>,
  ...
  ngOnInit() {
    ...
    for (let i = 0; i < this.data.array.prname.length; i++) {
      ...
      temptot = temptot + (qty * price);
    }
    this.subtot = temptot.toFixed(2);
    if (temptot > 350) {
      if (temptot > 600) {
        var dis = temptot * 7 / 100;
      } else {
        var dis = 0;
      }
      this.discount = dis.toFixed(2);
      this.total = (temptot - dis).toFixed(2);
    } else {
      this.total = (temptot + 20).toFixed(2);
    }
  }
...
export class OcStatusUpdateBottomSheetGlobal {
  ststusList = [];
  statusVal: any;
  constructor(
    public dialogRef: MatDialogRef<OcStatusUpdateBottomSheetGlobal>,
